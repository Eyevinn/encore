...
abstract class VideoEncodeTest<T : VideoEncode> {
    ...
    fun `vmafOpts returns null when vmaf is disabled`() {
        ...
        assertThat(encode.getOutput(defaultEncoreJob(), encodingProperties, FilterSettings())?.video).hasVmaf(null)
    }
    ...
    fun `vmafOpts returns null for segmented encode`() {
        ...
        assertThat(encode.getOutput(job, encodingProperties, FilterSettings())?.video).hasVmaf(null)
    }
    ...
    fun `vmafOpts adds seek filter when seekTo is set`() {
        ...
        val result = encode.getOutput(job, encodingProperties, FilterSettings())?.video?.vmaf
    ...
    fun `vmafOpts adds crop filter when cropTo is set`() {
        ...
        val result = encode.getOutput(defaultEncoreJob(), encodingProperties, FilterSettings())?.video?.vmaf
