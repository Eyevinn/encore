...
class AudioEncodeTest {
    ...
    fun `dialogue enhance Native stereo input emits builtin dialoguenhance filter`() {
        ...
        val filter = encode.getOutput(job(getAudioStream(2)), encodingProperties, FilterSettings())!!
    ...
    fun `dialogue enhance Dn 5dot1 input inserts dnenhance on FC channel`() {
        ...
        val filter = encode.getOutput(job(getAudioStream(6)), encodingProperties, FilterSettings())!!
            .audioStreams.single().filter
        assertThat(filter)
    ...
    fun `dialogue enhance Native mono input is rejected`() {
        ...
        val output = encode.getOutput(job(getAudioStream(1)), encodingProperties, FilterSettings())
