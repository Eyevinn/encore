...
class SegmentedEncodeService(
    ...
    fun segmentedEncodingInfo(encoreJob: EncoreJob): SegmentedEncodingInfo? {
        ...
        val audioEncodingConfig = audioEncodingMode(encoreJob, profile)
    ...
    fun audioEncodingMode(encoreJob: EncoreJob, profile: Profile): AudioEncodingConfig {
        ...
        val hasNonAacAudioEncode = profile.allAudioEncodes()
            .any { !listOf("aac", "libfdk_aac").contains(it.codec) }
        val audioEncodingMode = when {
     ...
     * - ENCODE_SEPARATELY_SEGMENTED: N AUDIOSEGMENT tasks + N VIDEOSEGMENT tasks
     */
